...
| [1.2](./1.2) | 0~9999 计数 + 四位数码管动态扫描 | `v1.2` | ⚠️ 待分配引脚 |
...
- **`.qsf` 里没有任何引脚分配**，`seg` / `seg_s` / `clk` / `reset_l` 都还没绑定到实际引脚，
  现在编译能过但下载到板子上不会有正确现象 —— 需要对照原理图补
  `set_location_assignment`。
- `clk_10hz` 由 `clk_200hz` 这个寄存器输出当时钟驱动，属于**衍生时钟**。
  入门够用，工程上应改成「时钟使能」写法：全部寄存器统一挂 50MHz，用 en 脉冲控制更新。
- `dyn_seg.v` 的组合逻辑 `always @(*)` 里用了非阻塞 `<=`，应为阻塞 `=`。
  能综合出正确电路，但仿真可能与综合不一致。
- 段码表按**共阴**数码管写（段位 = 1 点亮）。若板子是共阳，整表取反。
